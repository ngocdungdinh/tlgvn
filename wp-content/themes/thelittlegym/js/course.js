...
jQuery(document).ready(function(){
    ...
    /*jQuery('.wpcf7-form').unbind('DOMSubtreeModified').bind("DOMSubtreeModified",function(){
        ...
        var sendMailOk = self.find('.wpcf7-mail-sent-ok');
        if(sendMailOk.length > 0 && self.find('.wpcf7-validation-errors').length == 0){
            var wpcf7StyleHide = self.find('div[style="display: none;"]');

            self.find('div').addClass('hide');
            sendMailOk.removeClass('hide').css('margin', '0px');
            wpcf7StyleHide.removeClass('hide').css('display', 'none');
            self.find('wpcf7-mail-sent-ok');

            
        }else if(self.find('.wpcf7-validation-errors').length > 0){
            var wpcf7StyleHide = self.find('div[style="display: none;"]');
